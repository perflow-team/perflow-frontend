...
test('views, popularity and new releases use different stored catalog fields', () => {
  const books = [book(1, { views: 900, likes: 10 }), book(2, { views: 100, likes: 30 }),
    book(3, { views: 500, likes: 20, is_new: true }), book(4, { is_new: true })]
...
test('ties are deterministic, ratings remain separate, and limits apply after sorting', () => {
  const books = [book(3, { views: 100, likes: 10, rating: 9 }),
    book(2, { views: 200, likes: 10, rating: 8 }), book(1, { views: 200, likes: 10, rating: 8 })]
  assert.deepEqual(ids(rankNovels(books, 'popular', 2)), [1, 2])
...
test('ranking API works against the deployed legacy catalog without the unsupported ranking endpoint', async () => {
  ...
  const calls = []
  const books = [book(1, { views: 500, likes: 5 }), book(2, { views: 50, likes: 10, is_new: true })]
