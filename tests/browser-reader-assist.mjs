...
try{
 ...
 for(const [term] of terms){
  ...
  await page.getByRole('tooltip').getByText(`${term}의 자세한 설명입니다.`,{exact:true}).waitFor()
  assert.equal(lookups.length,calls+1,`${term}: hover loads one definition`)
  assert.equal(await page.getByRole('dialog').count(),0,`${term}: hover preview is non-modal`)
  await target.click()
  await page.getByRole('dialog').getByText(`${term}의 자세한 설명입니다.`,{exact:true}).waitFor()
  assert.equal(lookups.length,calls+1,'Click reuses the prepared hover result')
  assert.equal(lookups.at(-1).word,term)
  assert.equal(lookups.at(-1).current_char_offset, content.indexOf('.')+1)
  await closeCard(page)
  await target.hover();await page.waitForTimeout(500)
 ...
 console.log('PASS: hover previews all types, exact cutoff, cached click, Escape, Enter/Space activation')
 await openPanel(page,'용어사전')
 await page.locator('aside').getByRole('button',{name:'군산 장소',exact:true}).click()
 await page.locator('aside').getByText('군산의 자세한 설명입니다.',{exact:true}).waitFor()
