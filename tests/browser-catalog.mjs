...
const books = [
  { id: 2, title: '탁류', author: '채만식', views: 900, rating: 3.5, tags: ['인생하드모드'], genres: ['드라마'] },
  { id: 4, title: '동백꽃', author: '김유정', views: 700, rating: 4.9, tags: ['츤데레', '눈치제로남주'], genres: ['로맨스'] },
  { id: 5, title: '날개', author: '이상', views: 100, rating: 4.7, tags: ['의식의흐름'], genres: ['심리'] },
  ...
  for (const width of [320, 390, 1280]) {
    ...
    await page.route('**/api/**', route => {
      ...
      if (url.pathname === '/api/novels/ranking') {
        ...
        return json(url.searchParams.get('sort') === 'new' ? [] : [...candidates].sort((a, b) => b[url.searchParams.get('sort')] - a[url.searchParams.get('sort')]))
      }
    ...
    const ratings = page.getByRole('region', { name: '별점 랭킹', exact: true })
    ...
    await page.getByRole('region', { name: '오늘의 신작', exact: true }).getByText('오늘 새로 등록된 작품이 아직 없어요.').waitFor()
    assert.equal(await page.getByText('#츤데레').count(), 3)
    ...
    await page.getByRole('button', { name: '오늘의 신작', exact: true }).click()
    await page.getByText('오늘 새로 등록된 작품이 아직 없어요.').waitFor()
